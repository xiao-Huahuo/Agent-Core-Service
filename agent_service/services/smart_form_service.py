...
class SmartFormService:
    ...
    def __init__(self, *, engine: Engine, create_tables: bool = True) -> None:
        ...
        if create_tables:
            ...
            self._ensure_form_scope_columns()
            self._ensure_row_height_column()
            self._ensure_row_timestamp_columns()
            self._ensure_column_description_column()

    def _ensure_form_scope_columns(self) -> None:
        """为旧智能表格补充知识库归属和显式表格类型。"""

        columns = {column["name"] for column in inspect(self.engine).get_columns("smart_forms")}
        with self.engine.begin() as connection:
            if "library_id" not in columns:
                connection.execute(text("ALTER TABLE smart_forms ADD COLUMN library_id VARCHAR NOT NULL DEFAULT ''"))
            if "form_kind" not in columns:
                connection.execute(text("ALTER TABLE smart_forms ADD COLUMN form_kind VARCHAR NOT NULL DEFAULT 'literature'"))
    ...
    def _ensure_row_timestamp_columns(self) -> None:
        """为旧表格行补充稳定的入表和更新时间。"""

        columns = {column["name"] for column in inspect(self.engine).get_columns("smart_form_rows")}
        with self.engine.begin() as connection:
            if "created_at" not in columns:
                connection.execute(text("ALTER TABLE smart_form_rows ADD COLUMN created_at DATETIME"))
            if "updated_at" not in columns:
                connection.execute(text("ALTER TABLE smart_form_rows ADD COLUMN updated_at DATETIME"))
            connection.execute(text("UPDATE smart_form_rows SET created_at = CURRENT_TIMESTAMP WHERE created_at IS NULL"))
            connection.execute(text("UPDATE smart_form_rows SET updated_at = created_at WHERE updated_at IS NULL"))
