...
class LLMTaskScheduler:
    ...
    def _get_chat_model(
        ...
        *,
        ...
        if not resolved_api_key:
            logger.error("_get_chat_model: MISSING API KEY tier=%s model=%s has_api_key_param=%s has_small_api_key_param=%s",
                         model_tier, model_name, bool(api_key), bool(small_api_key))
        cache_key = (model_tier, tuple(sorted(tool_names)), float(final_temperature), float(timeout_seconds), resolved_api_key, resolved_base_url)
    ...
    def _resolve_model_runtime(
        ...
        *,
        ...
        api_key: 运行时大模型 API Key 覆盖,优先级高于 config。
        base_url: 运行时大模型 Base URL 覆盖,优先级高于 config。
        small_api_key: 运行时小模型 API Key 覆盖,优先级高于 config。
        small_base_url: 运行时小模型 Base URL 覆盖,优先级高于 config。
        """
