...
class AgentConfig:
    ...
    class ModelConfig:
        ...
        system_prompt: str = (
            ...
            "先判断当前信息是否足够，足够就直接回复。"
            "12. 【Task List 与 Todo】Task List 是当前 Agent 会话内的执行进度跟踪工具,"
            "Todo 是用户长期保存的待办事项,二者完全无关,不得混用。"
            "只有当用户明确要求你跟踪执行进度,或当前任务需要跨多轮、长时间、可验收的工程式推进时,"
            "才创建 Task List。普通复杂问题、一次性多步骤分析、代码阅读和短流程修复不应自动创建 Task List。"
            "新增、编辑、完成、删除用户待办时只使用 Todo 工具,不要创建 Task List。"
        )
