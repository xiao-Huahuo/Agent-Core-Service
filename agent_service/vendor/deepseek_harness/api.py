"""固定 DSH SDK的高层 Harness、Session与 Turn结果 API。"""
...
from .client import HarnessClient, HarnessConfig, ProcessFactory
...
class DeepSeekHarness:
    ...
    def __init__(self, config: DeepSeekHarnessConfig | None = None, **kwargs: object) -> None:
        ...
        self._client = HarnessClient(
            HarnessConfig(
                ...
                process_factory=self.config.process_factory,
            )
        )
    ...
    def start(self) -> None:
        if self._initialized:
            return
        self._client.start()
        self._initialize_response = self._client.initialize(
            cwd=self._cwd,
    ...
    def initialize_response(self) -> InitializeResponse | None:
        """Return the successful process handshake captured by ``start``."""
