...
export function apiAddTodo(userId: string, text: string, dueDate?: string): Promise<TodoItemData> {
  return apiPost(API_ROUTES.TODO_ADD, { user_id: userId, text, due_date: dueDate })
}
...
export function apiEditTodo(userId: string, todoId: string, text?: string, dueDate?: string): Promise<TodoItemData> {
  return apiPost(API_ROUTES.TODO_EDIT, { user_id: userId, todo_id: todoId, text, due_date: dueDate })
}
