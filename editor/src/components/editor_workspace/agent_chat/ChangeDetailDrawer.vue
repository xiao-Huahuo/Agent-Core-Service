...
<style scoped>
.change-detail{flex:0 0 min(390px,36vw);min-width:0;padding:var(--space-10);overflow:auto;background:var(--color-surface-raised);font-size:calc(11px * var(--font-scale))}header,.file-head{display:flex;align-items:center;gap:var(--space-8)}header{justify-content:space-between;margin-bottom:var(--space-10);color:var(--color-text-primary);font-weight:650}button{border:0;background:transparent;color:inherit;cursor:pointer}section{margin-bottom:var(--space-12)}.file-head{padding:5px 0;color:var(--color-text-secondary)}.file-head span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.file-head b{margin-left:auto;color:var(--color-success)}.file-head i{font-style:normal;color:var(--color-danger)}article{margin-top:var(--space-6);overflow:hidden;border-radius:var(--radius-sm)}
</style>
