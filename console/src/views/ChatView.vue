...
async function submitUserId() {
  ...
  userIdError.value = ''
  try {
    const profile = await setUserId(id)
    await sessionStore.load(profile.user_id)
  } catch (error) {
    userIdError.value = error instanceof Error ? error.message : 'user_id 初始化失败'
  } finally {
    ...
    <div v-if="!hasUserId" class="user-id-prompt">
      <div class="macos-card prompt-card">
        ...
        <div class="macos-card-body prompt-body">
          ...
          />
          <button class="prompt-btn" :disabled="!userIdInput.trim() || userIdLoading" @click="submitUserId">
            {{ userIdLoading ? 'Checking' : 'Enter' }}
          </button>
        </div>
        <p v-if="userIdError" class="prompt-error">{{ userIdError }}</p>
      </div>
    </div>
