...
## 2026-08-21 - 限制端到端测试资源占用

- 根因：`CI=1` 会将 Playwright 失败用例自动重试两次，同时默认端到端入口覆盖三个浏览器；在完整 Vite 构建后连续执行时，会将单次失败放大为多轮高内存任务。
- 修复：Playwright 全局固定为单 worker、零重试、首次失败即停止，并关闭常规 trace 留存。
- 新增 `npm run test:e2e:smoke`，固定仅使用 Chromium 且继承相同的资源上限，供局部界面改动验证使用。
- 约束：轻微样式改动不再以“完整构建 + 全浏览器端到端测试”作为默认验证路径。

## 2026-08-22 - Debug 工具注册表与全局常量
