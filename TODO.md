### TODOs
- [ ] 系统性的真正打通多模态解析链,包括图片OCR解析,扫描件pdf的图片渲染,pptx的渲染与预览等.
- [ ] 定时自动化任务： 可以给Agent布置定时自动化任务，比如每天晚上9点git commit之类的。
- [ ] 点击会话历史的加载过程中不应该瞬间回到欢迎页再加载对话区,而是应该显示一个加载中的loader.
- [ ] 工具条内容合并: 在前端将连续的相同工具调用的结果进行合并.
  - 重要依赖: 首先要手动的将前端工具条详情所显示的内容进行规范化.
  - "合并"的意思: 比如,多次联网搜索,就应该把工具条显示的"联网搜索: X条"的X进行加法,且出现"翻日历"的动效,使得这个X在下次同样的工具出现时增加,而不是写很多行工具条.工具条详情里面则是用横线切分不同次数的工具调用结果,合并展示.  再比如,对于终端工具,则工具条显示的是同种类终端指令叠加,例如"运行了powershell命令: python ... (换行) ls ... (换行) cat ...",工具条详情也是横线分割不同次的调用结果.
- [ ] 调试Skill，协调Skill要求与沙盒/完全访问模式的终端权限。
  - [x] 为Agent的每个Session对话都提供“导出”按钮，点击后以yaml导出会话的所有信息，包括：会话的元信息，用户提问，Agent工具调用条的文字，Agent工具条的详细内容，Agent的中间回答和最终回答。按顺序排列。
  - [x] 修复当Agent的工具条过长(比如Agent调用终端工具输入了较长的指令)时会导致Agent页面发生整体右移的bug，也有可能不是这个导致的，但是有时候就会出现页面发生整体右移的bug。
  - [ ] 权限解封: 为完全访问模式解禁外部工具调用限制(比如pandoc,不要硬编码); 为所有权限模式解禁命令段数和参数限制(比如dir /s /b 结果要求只能制定一个目录路径,ls -R结果ls不支持-R,这些限制是不必要的)
- [ ] 可统计的：
  - 工具总计调用次数
  - [ ] 会话内统计右边栏：
    - Dashboard里面理论上需要统计的是“长时统计结果”。因此，dashboard里面的三率饼图放到会话内统计右边栏，三率曲线图改成统计session为单位的，每次message思考耗时图放到会话内统计右边栏。
### IDEAs
- [ ] 娱乐功能：做一个桌宠，只需要悬停，可以右键，可以对话。
- [ ] 多agent:智能体蜂群,AgentSworm,父子Agent，子Agent可以是Loop，也可以是Graph。
### BUGs
- [ ] 图谱抽取逻辑有点小问题: 比如抽取一个"A喜欢B"的文档,发现只抽取了文档包含A和文档包含B,但是没有发现A和B之间的关联.
- [ ] 解决agent输出的代码块没渲染和HTML高亮乱码问题,尤其是修复输出大量含有这种乱码渲染导致页面卡死的问题.(&amp?,span?)

