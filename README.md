...
个人资料随机掉落在 Markdown、PDF、Office 文档、图片、表格和代码中。
...
组件按 all、buttons、checkboxes、toggle switches、cards、loaders、inputs、radio buttons、forms、patterns 和 tooltips 分类筛选，以自适应尺寸的"瀑布流"卡片展示，小型组件不会被强行撑大，较高或较宽的组件也会按照自身内容自然排布。
...
动态上下文拼装在**每次真实模型调用前**，依据当前模型的实际 token 容量决定本次请求放入哪些系统规则、工具定义、历史消息、工具结果、压缩摘要、Skill、附件与检索内容。它只生成本次请求的预算内表示，不修改或删除数据库中的完整消息和工具结果；完成迁移后，不再使用 900、240、200、6000 等彼此无关的字符数裁剪模型可见内容。
...
##### 语义知识图谱
在多模态文件灌库的路径中,当文档被解析为Markdown后,一路进行JSON切片入向量库,另一路则进行异步的小模型实体关系提取/

语义知识图谱提取各文档内的实体,用LLM(小模型)异步解析**文档内各实体**的关联,将知识库多模态文件的 frontmatter 转译为**实体-关系图**(ER图),最终持久化到 SQLite,前端通过 D3.js Canvas 实时渲染.
