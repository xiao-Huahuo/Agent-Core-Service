...
8. 可观测性: 配置一个前端,观测Agent在后台的一切行动,包括节点状态,上下文构建器的JSON,RAG召回的条目,召回筛选过程,会话摘要等. 配置完备的日志系统,所有的Agent行动也应该记录下来. 务必保证一切的信息传递过程的完全可视化.
9. 输出可定制性: 可以根据实际业务定制所需的字段,甚至可以定制display模式来控制输出的字段.
##  技术栈
- 版本: Python 3.12
- 微服务框架: FastAPI + gRPC
- 观测面板: Vue 3 + Pinia 
