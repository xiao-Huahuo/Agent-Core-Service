...
7. 可观测性：
   ...
   - Agent观测面板实时展示 Agent 行动轨迹，包括节点状态、上下文构建器、RAG 召回条目、召回筛选过程、会话摘要等。日志系统记录全部 Agent 行动，信息传递过程完全可视化。观测面板不重新推理业务结果，而是从会话消息、系统上下文快照、工具轨迹与模型返回元数据中派生统计值；统计口径按会话累计，同一 session 内不会因为图循环或单个节点结束而清零。
     ...
     * RAG 三率：统计来源包括两类，一类是上下文自动召回，另一类是知识库工具返回的召回条目。每次召回形成一个样本点，会话级指标取累计均值：$$Recall_t=\frac{1}{t}\sum_{i=1}^{t} recall_i,\quad HitRate_t=\frac{1}{t}\sum_{i=1}^{t} hit_i,\quad Confidence_t=\frac{1}{t}\sum_{i=1}^{t} confidence_i$$ 知识库工具召回没有显式分数时，用命中情况作为置信度兜底；其召回率按返回条数与请求上限估算：$$recall_i=\min(\frac{返回条数_i}{请求上限_i}\times100,100),\quad hit_i=\mathbb{1}(返回条数_i>0)\times100$$
