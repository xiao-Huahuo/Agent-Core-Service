...
个人资料散落在 Markdown、PDF、Office 文档、图片、表格和代码中。传统知识库擅长保存文件，却很难让 Agent 统一检索和操作这些不同结构的知识资产。
...
Agent 框架不能消除模型幻觉。MetaWeave 因此优先提供检索、引用、执行轨迹和调试观测，让用户能够核对依据并发现问题,这也是引用溯源功能的设计来源。
...
* 模型接入：支持用户配置的 OpenAI 兼容大小模型接口，并以内置 CPU `Qwen/Qwen3.5-2B` 提供未配置时的本地回退
* 关联数据库：SQLite
* 向量数据库：ChromaDB
...
多模态文档库是建立在文件系统上的集 "多模态转Markdown + Agent知识库记忆 + 知识图谱实体关系抽取" 为一体的,以 Obsidian 作为灵感来源,并进行**模态解析功能增强**和**Agent智能化集成**的高级文件系统.

##### 入库管理
"入库"的含义是将多模态的文件转换为统一的 **Markdown 中间层**，并按需要保存 frontmatter、文档内图片和其他附件。这些派生文件位于知识库的 `.mw/` 目录，由 MetaWeave 管理，编辑器预览、搜索、图谱和 Agent 检索共用这份解析结果。
...
- PDF 会根据文本、扫描页和表格等内容选择解析方式。常见图片可以提取 OCR 文字、图片说明和基础元数据；文档内嵌图片会保留所在段落、表格或页面附近的上下文。
- 没有专用解析器但可以解码的文件按纯文本处理。无法识别的二进制文件只登记资源信息，不进入语义索引。
...
“设置 → 存储管理 → 编译管理”会显示当前 LaTeX 发行版、系统安装或 MetaWeave 托管来源、版本、默认引擎、可用引擎、安装位置、文件数和磁盘占用。下载仓库、临时文件和编译缓存可以在存储路径中单独清理；MiKTeX 核心只能通过“卸载”操作移除。系统已有的 LaTeX 只读展示，MetaWeave 不会卸载或修改它。

![编辑区](docs/assets/多模态编辑区.png)

除常用 Markdown 语法外，还支持 Obsidian 风格的 Wiki 链接、嵌入链接和真正的反向链接查询。原生 `<img>` 会显示为独立图片块，`<iframe>` 媒体嵌入会显示为自适应 16:9 视频块。
编辑 Markdown 时可以使用 `[[文件]]` 建立 Wiki 链接，也可以写成 `[[文件|别名]]` 或 `[[文件#标题]]`。输入 `[[` 后会出现文件候选，继续输入文件名可以筛选；预览中的链接可以打开目标文件，带标题的链接会跳转到相应位置。

`![[文件]]` 使用相同的目标与别名规则，但预览时直接嵌入目标内容，并在左侧显示主题色竖线。Markdown 之间相互嵌入时最多递归五层。编辑器右键菜单提供“插入 Wiki 链接”和“插入嵌入链接”。

反向链接指向与 Wiki 链接相反的查询方向：打开某个 Markdown 文件后，可通过编辑区右键菜单的“显示反向链接”，或在“用户设置 → 外观”中启用同名选项。编辑区底部会按来源文件列出所有链接当前文件的文档，并保留每一处链接在原文中的完整写法；文件条目同时标记该链接指向整篇文章、某个标题或某个块。该开关按用户持久化，两处入口始终同步。
...
智能表格也是文献库的数据来源。每一行智能表格对应文献库中的一篇文献，所属表格则作为文献分组；在任一页面修改标题、标签、星级、智能字段或原文，另一处都会同步更新。普通表格不进入文献库。
...
- 表格可以(针对某一列或者全表格内)进行匹配搜索(和可选的语义搜索),高亮符合的行;可以筛选,筛选条例如下:
  ...
  - 星级筛选: 筛选1~5星的列.
- 表格分为普通表格和智能表格.普通表格只能导入或者手动改列,不再享有智能列功能.在智能表格中,如果用户选择删除文献列,那么就退化为普通表格.
- 表格可导出为csv文件(不导出文献列),也可以导出为xlsx,md表格(不导出文献列);还可以导出为zip,zip内包含导出的csv文件和文献列的所有文件.也可以从这几个格式中导入.
- 智能字段生成期间会遮盖旧内容并显示生成状态；LLM 返回空值或无效结果时保留原值，避免失败结果覆盖已有数据。
- 文献上传列会按单元格尺寸完整显示图片，PDF 则显示后端生成的第一页预览图。点击文献会在独立于 Agent 的编辑器侧边栏中打开原件，并保留编辑器按文件类型提供的 Preview、Markdown、Text、Forms 等查看模式。
- 在编辑器侧边栏保存可编辑文献时，该文件会被视为重新上传：系统重新灌库、刷新“文献内容”，并重新生成该行的全部智能字段。文件资源管理器双击文件、图书馆元文件和 MD-HTML 页面文件也复用同一个编辑器侧边栏，无需跳转离开当前页面。
- “文献内容”默认显示纯文本，展开后才渲染完整 Markdown；“图表”和“公式”无论折叠或展开都保持图片或KaTeX预览。表格支持连续矩形选区、沿任意行列边界拖动调整尺寸；纵向滚动时列名固定在顶部，横向滚动时文献上传列固定在左侧，表格总宽由实际列宽决定并可横向滚动。

#### 文献库

文献库是智能表格的阅读视图, 由智能表格中偏向“横向比较”的一行一列转换形式, 成为适合“逐篇阅读”的文献列表与编辑区.
注: 文献库与智能表格内部资料完全共享,每一文献与智能表格的行一一对应.文献仍归属于原来的智能表格，也继续使用原表格的字段和原始文件。

- 文献库只收录智能表格中的文献，普通表格不会出现在这里。一个知识库可以有多张智能表格，每张表格都可以包含多篇文献；用户可以选择某张表格查看，也可以选择“All”浏览当前知识库中的全部文献。
- 每个文献块对应智能表格中的一行，并显示封面或文件图标、标题、真实文件名、文献内容摘要和入表时间。展开后会按照原表格的列顺序展示全部字段。
...
注: 不建议使用本地模型作为主要模型,因为其输出很慢,不好用.
##### 大模型API设置
用户可在 "设置-LLM配置" 中输入联网大模型的模型名称和URL和API-Key.
用户未在设置页配置大模型的相关内容时,默认采用本地大模型作为大模型回答;
若用户设置了大模型API,则自动使用用户设置的作为大模型和小模型的回答来源(未配置小模型API时则用大模型API作为回退).
**建议采用`deepseek-v4-flash`作为大模型和小模型API**.
#### 智能体状态转移设计
...
##### 观测面板(debug)
Debug 页面用于直接检查 Agent 和后端运行时的真实数据。页面会加载当前 Session 的历史消息，并在“Agent 轨迹”“多模态入库”“记忆与知识”“工具注册表”“API”五个页签之间切换。

- **Agent 轨迹**：左侧按当前思考模式展示本轮 Agent 的语言轨迹和工具调用过程；右侧展示实际拼装给模型的上下文，可以在可读格式与 Raw 数据之间切换。可读视图会按顺序拆出上下文块，并统计拼装块数、总行数以及记忆和知识条目数。
- **多模态入库**：提供一套独立的知识库文件浏览器，可以前进、后退、返回上级、排序并查看文件入库状态。双击文件后，右侧会生成该文件的 Markdown 中间层、JSON 结构化结果、语义章节和带重叠窗口的最终切片，同时显示章节数、切片数和窗口/重叠参数。该过程只用于观测，不会向向量库写入记录；已有且未过期的结构化结果会直接复用。
...
##### 语义知识图谱
在多模态文件入库的路径中,当文档被解析为Markdown后,一路进行JSON切片入向量库,另一路则进行异步的小模型实体关系提取.
...
$$
\operatorname{fill}_i=\min\left(\frac{m_i+k_i}{M_i+K_i},1\right)\times100\%,\qquad
\operatorname{relevance}_i=\frac{1}{m_i+k_i}\sum_j q_{ij}\times100\%
$$
...
$$
\bar{x}_t=\frac{1}{t}\sum_{i=1}^{t}x_i,\qquad x\in\{\operatorname{fill},\operatorname{relevance},\operatorname{confidence}\}
$$
...
$$
u_i=\begin{cases}
\operatorname{total\_tokens}_i,&\operatorname{total\_tokens}_i>0\\
\operatorname{input\_tokens}_i+\operatorname{output\_tokens}_i,&\text{其他情况}
\end{cases}
$$
...
$$
U_{g,p}=\sum_{i\in g}\mathbf{1}(\operatorname{tier}_i=p)u_i,\qquad p\in\{\text{large},\text{small}\}
$$
...
$$
P(e)=\frac{N_e}{\sum_{j\in E_5}N_j}\times100\%,\qquad e\in E_5
$$
...
$$
a_e=\min\left(s_e,\max(C_{a(e)}-S_{a(e)},0)\right),\qquad
...
$$
T_k=\frac{1}{1000}\sum_n d_{k,n},\qquad
...
$$
\widehat{T}_k=\max\left(0.8,\ 0.45N_{trace}+0.35N_{tool}+\frac{\widehat{N}_{token}}{25}\right),\qquad
...
用户可以从文件右键菜单或独立的 Markdown-to-HTML 页面发起 HTML 可视化。对文件树或文件资源管理器中的文件选择“HTML 可视化”后，系统会先完成该文档的入库处理，再打开 HTML 可视化页面、收起文件树并展开 Agent 侧栏创建任务。多模态文档使用 Markdown 中间层生成页面，不直接依赖原始格式或解析 JSON。
