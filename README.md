...
- “文献内容”默认显示纯文本，展开后才渲染完整 Markdown。表格支持连续矩形选区、沿任意行列边界拖动调整尺寸；纵向滚动时列名固定在顶部，横向滚动时文献上传列固定在左侧，表格总宽由实际列宽决定并可横向滚动。
...
模型管理覆盖 Embedding、ReRank 和 PaddleOCR。每个模型首行显示配置名称、磁盘大小、下载/加载状态、当前是否启用和操作入口；展开后可以查看用途、完整路径、文件数、磁盘完整性、内存使用状态、模型仓库、OCR 语言和运行设备。Hugging Face 模型按仓库总字节和实际落盘字节显示真实百分比；PaddleOCR 等无法提前获得总量的任务只显示实时落盘字节和当前阶段，不生成假百分比。
...
[TODO]: 未来展望: 五库联合搜索,将结果一起放在同一个页面,形成震撼的不同组件同一页面.
