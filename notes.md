...
## 待确认
- 文件树右键菜单的共享样式或组件位置。
- 图谱节点事件、节点类型元数据与详情/编辑侧栏入口。
- 后端抽取关系的 schema、提示词、持久化及融合算法。

## 最终证据
- Canvas 新增真实节点右键命中事件；菜单顺序和视觉复用文件树间距、颜色、阴影、危险态与动效。
- 实体删除走后端持久化接口并删除全部入边/出边；文档清空保留文档节点，只删除该文档贡献的边和失去关联的实体。
- 单段与批量抽取提示均明确要求实体—实体多跳关系链；写入层只把文档连到关系根实体，链条 A→B→C 形成二层和三层。
- 实体 ID 与查询归并继续按规范化名称工作，三层集成测试确认不同类型的 `Alpha` 仅保留一个节点。
- 后端 22 + 2 项、前端 3 + 2 项、Chromium 1 项通过；定向 ESLint 0 错误，Vite 生产构建通过。
- 实际暗色界面确认文档菜单无截断、危险操作清晰、详情栏与编辑区侧边栏可同时正常呈现。

---

# Notes: 入库与图谱任务终止及顶部实时进度

## 待核对链路
- 入库任务取消 API 如何写入 `cancelling/cancelled`，store 轮询是否同步顶部统计和详情。
- 图谱队列是否具备逐任务取消信号，还是仅有应用关闭使用的全局 `_stop_event`。
- 图谱任务页行标识与后端队列 identity 是否稳定一致。
- 顶部两类进度条在终止态是否会立即保留终态消息再延迟收起。

## 已确认根因与实现
- 图谱队列原先把应用生命周期 `_stop_event` 直接传给每项任务，无法取消单项；现改为任务独立 `cancel_event`，应用关闭时再统一触发全部任务信号。
- 图谱取消需要覆盖等待中与运行中：等待项同步从 deque 移除并发布 `cancelled`；运行项先发布 `cancelling`，执行器检查信号后归一为 `cancelled`。
- 入库取消 API 已实时返回，但持久任务轮询在活动项消失后丢失本批身份，顶部百分比回到 0；活动任务现在加入前端跟踪集合，终态按队列完成度计为 100%，同时保留文件名和“已中止”详情。
- 顶部入库条原先只有轨道和百分比；现复用图谱条的信息密度，显示完成计数、文件名、阶段与阶段计数。
